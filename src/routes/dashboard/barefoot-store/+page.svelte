<script lang="ts">
	...
	import { barefootStorePathway } from '$lib/dashboard/barefoot-store';
...
<svelte:head>
	<title>{m.dashboard_barefoot_meta_title()}</title>
	<meta name="description" content={m.database_project_barefoot_store_description()} />
</svelte:head>
	...
	<div class="dashboard-detail__wrap">
		...
		<header class="dashboard-detail__header">
			<div class="dashboard-detail__header-copy">
				...
				<h1>{m.home_database_barefoot()}</h1>
				<p class="dashboard-detail__description">
					{m.database_project_barefoot_store_description()}
				</p>
			</div>

			<img
				class="dashboard-detail__image"
				src={barefootStoreImage}
				alt={m.home_database_barefoot_image_alt()}
			/>
		</header>
		...
		<div class="dashboard-detail__pathway">
			{#each barefootStorePathway as phase, index (phase.name)}
